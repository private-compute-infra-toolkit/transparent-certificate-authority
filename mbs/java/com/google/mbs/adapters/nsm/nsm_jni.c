...
JNIEXPORT jint JNICALL Java_com_google_mbs_adapters_nsm_NitroSecurityModule_init(JNIEnv *env, jclass ignored)
{
    return nsm_lib_init();
}

JNIEXPORT void JNICALL Java_com_google_mbs_adapters_nsm_NitroSecurityModule_exit(JNIEnv *env, jclass ignored, jint fd)
{
    nsm_lib_exit(fd);
}

JNIEXPORT jobject JNICALL Java_com_google_mbs_adapters_nsm_NitroSecurityModule_getAttestationDoc(
    JNIEnv *env, jclass ignored, jint fd, jbyteArray user_data_array, jbyteArray nonce_array, jbyteArray public_key_array)
{
    jclass nsm_result_class = (*env)->FindClass(env, "com/google/mbs/adapters/nsm/NsmJniResult");
